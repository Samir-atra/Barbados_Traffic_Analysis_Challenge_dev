"""Module for inference using the trained Forward-Forward algorithm model (Local Version).
...
# Restrict to a single GPU if needed, or use CPU if no GPU available
...
def main():
    ...
    model_path = os.path.join(base_dir, "models/ff_model.keras") 
    ...
    if not os.path.exists(model_path):
        ...
        # Optional: Try looking in current directory
        alt_path = "ff_model.keras"
    ...
    output_path = os.path.join(submissions_dir, "submission_local.csv")
